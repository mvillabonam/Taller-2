...
rf_model_2 <- ranger(
  Pobre ~ tasa_dependencia + Nper + median_education + hacinamiento + 
          costo_vivienda + vulnerabilidad + recibe_subsidios+
          num_ocupados+tipo_posesion+num_cuartos,
  data = train_dataset,
  num.trees = 500,
  mtry = floor(sqrt(length(predictors))),
  importance = 'impurity',
...
# Variable importance
var_importance <- importance(rf_model_2)
var_importance_df <- data.frame(
  Variable = names(var_importance),
  Importance = var_importance
)
...
# Define the parameter grid for AdaBoost.M1
adagrid <- expand.grid(
  mfinal = c(50, 200, 500),
  maxdepth = c(1, 2, 3, 4),
...
varImp_result <- varImp(adaboost_model)
plot(varImp_result, main = "Variable Importance")

# Make predictions on the test set
predictions <- predict(adaboost_model, newdata = test_dataset)

# Calculate probabilities
pred_probs <- predict(adaboost_model, newdata = test_dataset, type = "prob")
